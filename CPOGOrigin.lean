...
def contentSourceObs {Event Content Source : Type} :
    Unit → PossibilityToken Event Content Source → (Content × Source)
  | _, t => (t.content, t.source)
...
def originObs {Event Content Source : Type} :
    Unit → PossibilityToken Event Content Source → Event
  | _, t => t.origin
...
def policyObs {I Event Content Source : Type}
    (P : I → Source → Bool) :
    I → PossibilityToken Event Content Source → (Content × Bool)
  | i, t => (t.content, P i t.source)
