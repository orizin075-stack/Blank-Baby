...
theorem firstCommitClassDot2Failure
    ...
    Dia R (Box R (FirstCommitPA F A)) r /    Not (Box R (Dia R (FirstCommitPA F A)) r) :=
...
theorem splitAbstractionModalContrast :
    Not (DirectedAt splitRG .ri) /    DirectedAt
      (quotientModel splitModel splitPresentation).rG
      (splitPresentation.classOf .ri) /    (forall p : SplitClass -> Prop,
      ...
      Box (quotientModel splitModel splitPresentation).rG
        ...
        (splitPresentation.classOf .ri)) /    (Satisfies (quotientModel splitModel splitPresentation)
        (splitPresentation.classOf .rf) splitAntecedent /      Not (Satisfies (quotientModel splitModel splitPresentation)
...
theorem finiteMaxRealization
    ...
    exists order,
      SameSupport order batch /      RTC (MaxStep admissible) base (base ++ order) :=
...
theorem maxSealAndConservativity
    ...
    SealComplete E.closure /    forall (phi : Formula Atom) (x : CoreWorld),
      Satisfies C x phi <->
...
theorem possibilityRoleSeparation
    ...
    M.live ≠ M.believed /    M.committed ≠ M.believed /    M.committed ≠ M.live :=
...
theorem epistemicRoleDoesNotForceMetaphysical :
    PossibilityRoleAdequate cpogRoleModel /    Not (BridgeToMetaphysical cpogRoleModel roleMetFalse) :=
